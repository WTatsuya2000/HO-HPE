...
ベクトルのリストを返します。
"""
function all_combinations_multi_types(l::Int, Nt::Int)
    @assert iseven(l-Nt) "Error: l must be even (got l-Nt=$(l-Nt))."
    k = div(l-Nt,2)
...
"""
function count_combinations(l::Int, Nt::Int)
    @assert iseven(l-Nt) "Error: l-Nt must be even (got l-Nt=$(l-Nt))."
    k = div(l-Nt, 2)
    result = Vector{Vector{Int}}()
    ...
    for Na in 0:k, Nb in 0:k, Nc in 0:k
        ...
        if zero_count <= 4 && Na <= Nb && Nb <= Nc && Nc <= Nd
...
"""
function classify_loop(L,type_N::Int, Nt::Int)
    # L = 8
    CL = add_minus.(all_combinations_multi_types(L,Nt))
    NCL = length(CL)
    ...
    appending = Symbol[]
    for _=1:Nt
        push!(appending, Symbol("d"))
    end

    Type_loop = Dict()
    for i in 1:length(count_combinations(L,Nt))
        Type_loop[Symbol(i)] = []
    end

    for i in 1:length(count_combinations(L,Nt)), j in 1:length(CL_set)
        if CL_set[j][2] == count_combinations(L,Nt)[i]
            push!(Type_loop[Symbol(i)], vcat(CL_set[j][1],appending))
        end
    end
    if length(Type_loop) == 0
        println("L-Nt==0, Type_loop is empty.")
        Type_loop[Symbol("1")] = appending
    end
    return Type_loop
    perms_result = []
    is_new = true
    c = 0
    dir_check(joinpath(@__DIR__,"..","logs","log_c","L$L"))
    log = open(joinpath(@__DIR__,"..","logs","log_c","L$L","log$(L)_$(type_N)_c.txt"),"w")          
        println(log, "start", )
    close(log)
    ...
    @time for p1 in Type_loop[Symbol(type_N::Int)]
        ...
        for p in perms_new2
            log = open(joinpath(@__DIR__,"..","logs","log_c","L$L","log$(L)_$(type_N)_c.txt"),"a")
...
# ── 並列ユニーク化＋ログ出力 ──────────────────────────────────────────────────
function parallel_unique(perms::Vector{Vector{Symbol}}, L::Int, Nt::Int, log_path::String)
    n_threads = nthreads()
    @show n_threads 
...
function classify_loop(L::Int, type_N::Int, Nt::Int)
    # ログファイル名
    log_path = joinpath(@__DIR__,"..","logs","log_c","log_$(L)_$(type_N)_c.txt")
    # ログファイル初期化
    ...
    combos = count_combinations(L,Nt)
    ...
    Type_loop = Dict()
    if length(combos) == 0
        println("L-Nt==0, combos is empty.")
        Type_loop[Symbol("1")] = [appending]
        return Type_loop
    else
        ...
        for (arr, counts) in CL_set
            ...
            push!(Type_loop[Symbol(idx)], vcat(arr,appending))
        end
    end
    # return Type_loop
    ...
    # return Type_loop[Symbol(type_N)]
    @time for (grp_i, p1) in enumerate(Type_loop[Symbol(type_N)])
        ...
        if grp_i == 1
            ...
            # (2) 並列ユニーク化＋ログ
            perms_new3 = parallel_unique(perms_new2, L,Nt, log_path)
...
# # using Base.Threads
# global L = 10
# println("L=$L の組み合わせ数: ", length(count_combinations(L)))
# for i=1:length(count_combinations(L))
#     result = classify_loop(L,i)
#     dir_check("./L$L")
#     dat =open("./L$L/type$i","w")
#     for i in 1:length(result)
#         println(dat,String.(result[i]))  # 結果を文字列に変換して表示
#     end
#     close(dat)
# end
